...
def test_get_user(client, db, user, admin_headers):
    # test 404
    fake_user_url = url_for("api_v1.get_user", user_id="100000")
    bad_rep = client.get(fake_user_url, headers=admin_headers)
    assert bad_rep.status_code == 404
...
def test_put_user(client, db, user, admin_headers):
    # test 404
    user_url = url_for("api_v1.get_user", user_id="100000")
